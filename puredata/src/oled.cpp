...
void oled_fresh(t_oled *oled){
    ...
    SETFLOAT(oled->a_table+oled->a_table_i+3,(float)(CMD_SYSEX_ID[3]));
...
void oled_onCommand(t_oled *oled, t_symbol *s, int argc, t_atom *argv){
    ...
    if(s == gensym("reset_bytes")){
        ...
        SETFLOAT(oled->a_table+oled->a_table_i+1,1.f);
        SETFLOAT(oled->a_table+oled->a_table_i+2,atom_getfloat(&argv[0]));
        oled->a_table_i+=3;
    }
    else if(s == gensym("scale") || s == gensym("S")){
        SETFLOAT(oled->a_table+oled->a_table_i,(float)(CMD_SYMBOL_SCALE));
        SETFLOAT(oled->a_table+oled->a_table_i+1,1.f);
        SETFLOAT(oled->a_table+oled->a_table_i+2,atom_getfloat(&argv[0]));
        oled->a_table_i+=3;
    }
        ...
        SETFLOAT(oled->a_table+oled->a_table_i,(float)(CMD_SYMBOL_RECT));
        SETFLOAT(oled->a_table+oled->a_table_i+1,5.f);
        SETFLOAT(oled->a_table+oled->a_table_i+2,atom_getfloat(&argv[0]));
        SETFLOAT(oled->a_table+oled->a_table_i+3,atom_getfloat(&argv[1]));
        SETFLOAT(oled->a_table+oled->a_table_i+4,atom_getfloat(&argv[2]));
        SETFLOAT(oled->a_table+oled->a_table_i+5,atom_getfloat(&argv[3]));
        SETFLOAT(oled->a_table+oled->a_table_i+6,float(1));
        oled->a_table_i+=7;
    }  
        ...
        SETFLOAT(oled->a_table+oled->a_table_i+4,atom_getfloat(&argv[2]));
        SETFLOAT(oled->a_table+oled->a_table_i+5,atom_getfloat(&argv[3]));
        SETFLOAT(oled->a_table+oled->a_table_i+6,float(0));
        oled->a_table_i+=7;
    } 
        ...
        SETFLOAT(oled->a_table+oled->a_table_i+4,atom_getfloat(&argv[2]));
        SETFLOAT(oled->a_table+oled->a_table_i+5,atom_getfloat(&argv[3]));
        SETFLOAT(oled->a_table+oled->a_table_i+6,atom_getfloat(&argv[4]));
        oled->a_table_i+=7;
    }
    else if((s == gensym("line") || s == gensym("l")) && argc == 4){
        SETFLOAT(oled->a_table+oled->a_table_i,(float)(CMD_SYMBOL_LINE));
        SETFLOAT(oled->a_table+oled->a_table_i+1,4.f);
        SETFLOAT(oled->a_table+oled->a_table_i+2,atom_getfloat(&argv[0]));
        SETFLOAT(oled->a_table+oled->a_table_i+3,atom_getfloat(&argv[1]));
        SETFLOAT(oled->a_table+oled->a_table_i+4,atom_getfloat(&argv[2]));
        SETFLOAT(oled->a_table+oled->a_table_i+5,atom_getfloat(&argv[3]));
        ...
        SETFLOAT(oled->a_table+oled->a_table_i,(float)(CMD_SYMBOL_STRING));
        //byte length calculated later after counting string len
        SETFLOAT(oled->a_table+oled->a_table_i+2,atom_getfloat(&argv[0]));
        SETFLOAT(oled->a_table+oled->a_table_i+3,atom_getfloat(&argv[1]));
        
        int chars = 0;
        for(int word=2; word<argc; word++){
            atom_string(&argv[word],oled->str,128);
            for(int letter=0; letter<128; letter++){
                char cc = oled->str[letter];
                if(cc == 0) break;
                SETFLOAT(oled->a_table+oled->a_table_i+4+chars,float(cc));
                chars++;
            }
            SETFLOAT(oled->a_table+oled->a_table_i+4+chars,float(' '));
            chars++;
        }
        //byte len calculation
        SETFLOAT(oled->a_table+oled->a_table_i+1,float(chars + 2 - 1));
        oled->a_table_i += chars + 2 + 1;
    }
